...
{
    wlcs::CheckInterfaceExpected expect_foreign_supported{the_server(), zwlr_foreign_toplevel_manager_v1_interface};
};
...
TEST_F(ForeignToplevelManagerTest, supports_zwlr_foreign_toplevel_manager_v1)
{
...
TEST_F(ForeignToplevelHandleTest, can_minimize_foreign)
{
    ...
    ASSERT_THAT(client.focused_window(), Eq((wl_surface*)surface)) << "precondition failed";
    ...
    EXPECT_THAT(client.focused_window(), Ne((wl_surface*)surface))
        << "surface under pointer when it should have been minimized";
    EXPECT_THAT(client.focused_window(), Eq((wl_surface*)below_surface))
...
TEST_F(ForeignToplevelHandleTest, can_unminimize_foreign)
{
    ...
    ASSERT_THAT(client.focused_window(), Eq((wl_surface*)below_surface)) << "precondition failed";
    ...
    EXPECT_THAT(client.focused_window(), Ne((wl_surface*)below_surface))
